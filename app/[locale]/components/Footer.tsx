...
export default function Footer() {
  ...
  const serviceLinks = [
    { label: t.serviceLinks[0], href: getSlug('web-solutions', locale) },
    { label: t.serviceLinks[1], href: getSlug('seo', locale) },
  ];

  const companyLinks = [
    { label: t.companyLinks[0], href: getSlug('about', locale) },
    { label: t.companyLinks[1], href: getSlug('blog', locale) },
    { label: t.companyLinks[2], href: getSlug('contact', locale) },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-sm text-gray-400 mb-4 leading-relaxed">{t.tagline}</p>
            ...
            <div>
              <h4 className="text-white font-semibold text-sm mb-3">{t.followUs}</h4>
              <div className="flex items-center space-x-3">
                <a href="https://www.instagram.com/sitelab.lt" target="_blank" rel="noopener noreferrer"
                ...
                <a href="https://www.facebook.com/sitelab.lt" target="_blank" rel="noopener noreferrer"
          ...
          <div>
            ...
            <ul className="space-y-3">
              {serviceLinks.map((link, i) => (
                <li key={i}>
                  <Link href={`/${locale}/${link.href}`} className="text-gray-400 hover:text-[#43ffae] transition-colors text-sm flex items-center group">
          ...
          <div>
            ...
            <div className="mt-6 pt-6 border-t border-gray-800">
              <h4 className="text-white font-semibold text-sm mb-3">{t.hours}</h4>
              <p className="text-xs text-gray-500">{t.hours1}</p>
              <p className="text-xs text-gray-500">{t.hours2}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li>
                <a href="mailto:info@sitelab.lt" className="group flex items-start space-x-3 text-gray-400 hover:text-[#43ffae] transition-colors">
              ...
              <li>
                <a href="tel:+37060267098" className="group flex items-start space-x-3 text-gray-400 hover:text-[#43ffae] transition-colors">
            ...
            <Link href={`/${locale}/${getSlug('contact', locale)}`} className="mt-6 inline-block w-full text-center py-3 px-6 bg-gradient-to-r from-[#13aff0] to-[#43ffae] text-white rounded-lg font-semibold text-sm hover:scale-105 transition-transform hover:shadow-lg hover:shadow-[#13aff0]/50">
              {t.quote}
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-sm text-gray-500 text-center md:text-left">
              © {currentYear} SiteLab. {t.rights}
            </p>
            <div className="flex items-center space-x-6">
              <Link href={`/${locale}/${getSlug('privacy', locale)}`} className="text-sm text-gray-500 hover:text-[#43ffae] transition-colors">{t.privacy}</Link>
              <Link href={`/${locale}/${getSlug('terms', locale)}`} className="text-sm text-gray-500 hover:text-[#43ffae] transition-colors">{t.terms}</Link>
            </div>
          </div>
        </div>
      </div>
